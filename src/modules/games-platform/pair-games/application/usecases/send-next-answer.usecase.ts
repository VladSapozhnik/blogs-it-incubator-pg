...
export class SendNextAnswerUseCase implements ICommandHandler<SendNextAnswerCommand> {
  ...
  async execute({ userId, answer }: SendNextAnswerCommand): Promise<string> {
    ...
    const getAnswers: PlayerAnswer[] =
      await this.playerAnswerRepository.getPlayerAnswer(activeGame.id, userId);
    ...
    const currentQuestion: QuizQuestion = questions[getAnswers.length];
